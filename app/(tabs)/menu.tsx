...
const DEFAULT_ICON = {
  name: "storefront-outline" as any,
...
export default function MenuScreen() {
  ...
  // ✅ ListHeader — walang search bar dito, categories + sort + empty state lang
          ...
          {allCats.map((cat) => {
            ...
            return (
              <TouchableOpacity
                ...
                onPress={() =>
                  setActiveCategory(cat.name === "All" ? "" : cat.name)
                }
                activeOpacity={0.75}
              >
                <Ionicons
                  name={icon.name}
                  size={14}
                  color={active ? icon.color : Colors.text3}
                />
                <Text
                  style={[
                    styles.catPillText,
                    { color: active ? icon.color : Colors.text2 },
                  ]}
                >
                  {cat.name}
                </Text>
  ...
  const renderProduct = useCallback(
    ({ item: p, index }: { item: any; index: number }) => {
      ...
      return (
        ...
        >
          <View style={[styles.productImgWrap, { height: imgHeight }]}>
            <Image
              source={{
                uri:
                  p.image ||
                  `https://placehold.co/300x200/1E1E1E/E8462A?text=${encodeURIComponent(p.name)}`,
              }}
          ...
          <View style={styles.productBody}>
            <Text style={[styles.productCat, isLarge && { fontSize: 11 }]}>
              {p.category}
            </Text>
            <Text
              style={[styles.productName, isLarge && { fontSize: 16 }]}
              numberOfLines={1}
            >
              {p.name}
            </Text>
            <Text
              style={[styles.productDesc, isLarge && { fontSize: 12 }]}
              numberOfLines={2}
            >
              {p.description}
            </Text>
            <View style={styles.productFooter}>
              <Text style={[styles.productPrice, isLarge && { fontSize: 17 }]}>
                ₱{p.price?.toFixed(2)}
              </Text>
              {p.prepTime && (
                <View style={styles.prepTimeWrap}>
                  ...
                  />
                  <Text
                    style={[styles.productTime, isLarge && { fontSize: 11 }]}
                  >
                    {p.prepTime}min
                  </Text>
                </View>
  ...
  return (
    <SafeAreaView style={styles.safe} edges={["top"]}>
      {/* ✅ STATIC na header — hindi nire-remount, kaya hindi nag-close ang keyboard */}
      ...
      {/* ✅ Search bar LABAS ng FlatList — stable, hindi nire-remount */}
